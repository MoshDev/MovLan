...
class MoviesSearchDataSource(
  ...
  override fun loadInitial(
    params: LoadInitialParams<SearchQuery>,
    callback: LoadInitialCallback<SearchQuery, Movie>
  ) {
    GlobalScope.launch {
      val page = 1
      repository.searchMovies(query, page, ::sortByPopularity) {
        when (it) {
          is Maybe.Some -> callback.onResult(
            it.value.results!!,
            null,
  ...
  override fun loadAfter(
    params: LoadParams<SearchQuery>,
    callback: LoadCallback<SearchQuery, Movie>
  ) {
    GlobalScope.launch {
